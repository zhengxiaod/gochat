...
func (c *Client) SyncInPB() {
	c.SendMsgInPB(pb.CmdType_CT_Sync, &pb.SyncInputMsg{Seq: c.seq})
}
...
func (c *Client) Receive() {
	for {
		...
		c.HandlerMessageInPB(bytes)
	}
}

// HandlerMessage 客户端消息处理
func (c *Client) HandlerMessageInPB(bytes []byte) {
	fmt.Println("收到顶层 OutPut 消息：", string(bytes))
	msg := new(pb.Output)
...
func (c *Client) LoginInPB() {
	...
	c.SendMsgInPB(pb.CmdType_CT_Login, loginMsg)
}

// SendMsgInPB 客户端向服务端发送上行消息
...
// ReadLineInPB 读取用户消息并发送
func (c *Client) ReadLineInPB() {
	var (
		msg         string
		receiverId  uint64
		sessionType int8
